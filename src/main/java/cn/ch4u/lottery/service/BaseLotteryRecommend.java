...
public abstract class BaseLotteryRecommend implements ILotteryRecommend{
    /**
     * 通用推荐
     * @param typeEnum 类型
     * @param list 结果集
     * @param isDesc 是否为降序推荐
     * @return
     */
    protected RecommendRes commonRec(LotteryTypeEnum typeEnum, List<Record> list,boolean isDesc){
        ...
        for (int i = 1; i <= parts; i++) {
            ...
            if(porder){
                ...
                if (prepeat){
                    ...
                    //map根据value排序
                    Map<Integer,Integer> smap= LotteryUtil.sortMapByValues(map,isDesc);
